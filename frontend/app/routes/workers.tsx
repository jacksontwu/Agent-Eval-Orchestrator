  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-lg font-semibold">机器</h1>
        ...
        <table className="w-full text-sm">
          <thead className="bg-white/[0.04] text-left text-slate-400">
            <tr>
              ...
              <th className="px-4 py-2 font-medium">槽位</th>
          ...
          <tbody>
            {data?.workers.map((w) => (
              <tr key={w.workerId} className="border-t border-white/5">
                <td className="px-4 py-2">{w.displayName}<div className="text-xs text-slate-500">{w.host}</div></td>
                <td className="px-4 py-2"><Badge tone={statusTone(w.status)}>{w.status}</Badge></td>
                <td className="px-4 py-2 text-slate-300">{w.slotsUsed}/{w.slotsTotal}</td>
            ...
            {data?.workers.length === 0 && (
              <tr><td className="px-4 py-3 text-slate-500" colSpan={5}>暂无机器</td></tr>
            )}
          </tbody>
        </table>
      </Card>
...
function AddMachineDialog({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black/60 p-4" onClick={onClose}>
      <Card className="max-w-2xl space-y-3" onClick={(e) => e.stopPropagation()}>
        <h2 className="text-base font-semibold">添加机器</h2>
        <p className="text-sm text-slate-400">在目标机器上执行下面的命令完成自注册：</p>
        <pre className="overflow-x-auto rounded-md bg-black/40 p-3 text-xs text-emerald-300">{command}</pre>
